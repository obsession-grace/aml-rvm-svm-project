...
def regression():
    ...
    for noise in noises:
        for _ in range(iterations):
            ...
            average_number_of_relevance_vectors = average_number_of_relevance_vectors + len(relevance_vectors) / iterations
            #Get predictions.
            x_input = np.linspace(-10, 10, 1000)
            #t_predictions = get_predictions(rvm, x_input)
            t_predictions = rvm.make_prediction(np.vstack(x_input))
            #Compute error
            y_true = np.sin(x_input) / x_input
            average_error = average_error + (np.sum(np.abs(y_true - t_predictions)) / len(x_input)) / iterations
    ...
    for iteration in range(iterations):
        ...
        #Fit data.
        rvm = RVM(method="regression", kernel_type="linear")
        rvm.train(train_samples, train_targets)
        #Get relevance vectors
        average_number_of_relevance_vectors = average_number_of_relevance_vectors + len(rvm.relevance_vectors) / iterations
        #Make predictions on test samples.
        t_predictions = rvm.make_prediction(test_samples)
        #Compute error.
        average_error = average_error + (np.sum(np.abs(test_targets - t_predictions)) / len(test_targets)) / iterations
